...
export default async function TesKamAttemptPage() {
  ...
  const test = await getTestForAttempt("KAM");
  return (
    <>
      <PageHeader title="Tes KAM" description="Pilih jawaban terbaik untuk setiap soal." />
      <TestAttemptForm questions={test.questions} durationMinutes={test.durationMinutes} submitEndpoint="/api/student/kam/submit" resultPath="/mahasiswa/tes-kam/result" />
    </>
  );
}
