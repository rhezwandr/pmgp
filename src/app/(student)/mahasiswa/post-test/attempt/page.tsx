...
export default async function PostTestAttemptPage() {
  ...
  const test = await getTestForStudentAttemptSecure("POST_TEST");
  return (
    <>
      <PageHeader title="Post Test" description="Kerjakan semua soal untuk memperoleh hasil akhir." />
      <TestAttemptForm questions={test.questions} durationMinutes={test.durationMinutes} submitEndpoint="/api/student/post-test/submit" resultPath="/mahasiswa/post-test/result" shuffleSeed={student.id + "POST_TEST"} testType="POST_TEST" />
    </>
  );
}
